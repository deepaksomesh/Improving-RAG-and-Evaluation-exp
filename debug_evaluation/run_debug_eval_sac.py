...
from sac_rag.utils.ai import AIEmbeddingModel, AIModel
...
class NumpyBaselineRetrievalMethod(BaselineRetrievalMethod):
    ...
    async def sync_all_documents(self) -> None:
        ...
        # Add summarization parameters if present in strategy
        if hasattr(self.retrieval_strategy.chunking_strategy, 'summary_model') and self.retrieval_strategy.chunking_strategy.summary_model:
        ...
        # For SAC, get_chunks already prepends the summary if using summary_rcts
        chunk_contents = [
            chunk.content 
...
async def main(args):
    ...
    # 1. Define SAC Strategy (summary_rcts)
    strategy = BaselineRetrievalStrategy(
        ...
        chunking_strategy=ChunkingStrategy(
            ...
            summary_model=AIModel(company="openai", model="gpt-oss-20b"), # Adjust based on credentials if needed, but caches exist
            summary_prompt_template="System: You are an expert legal document summarizer. User: Summarize the following legal document text. Focus on extracting the most important entities, core purpose, and key legal topics. The summary must be concise, maximum {target_char_length} characters long, and optimized for providing context to smaller text chunks. Output only the summary text. Document:\n{document_content}",
            prompt_target_char_length=200,
            summary_truncation_length=220,
            use_cache=True
        ),
    ...
    for dataset_name, queries_results in results_by_dataset.items():
        ...
        log_path = os.path.join(os.path.dirname(args.log_file), f"debug_log_sac_{dataset_name}.txt")
        with open(log_path, 'w', encoding='utf-8') as log_f:
            log_f.write(f"=== Debug RAG Evaluation Run ===\n")
            log_f.write(f"Dataset: {dataset_name}\n")
            log_f.write(f"Method: Summary-Augmented Chunking (SAC)\n")
...
class Logger(object):
    def __init__(self, filename="debug_evaluation/debug_log_sac.txt", stream=sys.stdout):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log-file", type=str, default="debug_evaluation/debug_log_sac.txt")
