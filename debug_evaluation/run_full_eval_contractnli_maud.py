...
# Import the existing evaluation logic and data loader to stay consistent
from benchmarks.legalbenchrag.run_benchmark import setup_and_load_data, QAResult, BenchmarkResult
...
async def main(args):
    ...
    import benchmarks.legalbenchrag.run_benchmark
    benchmarks.legalbenchrag.run_benchmark.benchmark_name_to_weight = {
        "contractnli": 0.5, "maud": 0.5
    }
    ...
    for dataset_name, queries_results in results_by_dataset.items():
        ...
        log_path = os.path.join(os.path.dirname(args.log_file), f"debug_log_{dataset_name}.txt")
...
class Logger(object):
    def __init__(self, filename="debug_evaluation/debug_log.txt", stream=sys.stdout):
...
if __name__ == "__main__":
    ...
    # Defaulting to 0 for full evaluation
    parser.add_argument("--max-tests", type=int, default=0, help="Number of queries to run")
    parser.add_argument("--log-file", type=str, default="debug_evaluation/full_log.txt")
