...
class ColBERTRetrievalMethod(RetrievalMethod):
    def __init__(self, retrieval_strategy):
        ...
        self.all_chunks = []
        try:
            # --- START MONKEY PATCH FOR WINDOWS ---
            import colbert.modeling.colbert
            # Disable loading the C++ extension (segmented_maxsim.cpp) which requires MSVC (cl.exe) on Windows
            colbert.modeling.colbert.ColBERT.try_load_torch_extensions = lambda use_gpu: None

            # Provide a pure Python/PyTorch fallback for segmented_maxsim
            @staticmethod
            def python_segmented_maxsim(scores, D_lengths):
                from colbert.modeling.strided_tensor import StridedTensor
                from colbert.modeling.colbert import colbert_score_reduce, ColBERTConfig
                scores_padded, scores_mask = StridedTensor(scores, D_lengths, use_gpu=False).as_padded_tensor()
                return colbert_score_reduce(scores_padded, scores_mask, ColBERTConfig())

            colbert.modeling.colbert.ColBERT.segmented_maxsim = python_segmented_maxsim
