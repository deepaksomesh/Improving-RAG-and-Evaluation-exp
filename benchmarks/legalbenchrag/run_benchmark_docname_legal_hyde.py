...
from sac_rag.utils.hyde_specific import generate_specific_legal_hypothesis
...
async def run_strategy(
    ...
    async def run_query(qa_gt: QAGroundTruth) -> Tuple[QAGroundTruth, List[RetrievedSnippet]]:
        async with query_semaphore:
            ...
            try:
                hypothesis = await generate_specific_legal_hypothesis(qa_gt.query, dataset_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results-dir", type=str, default="./results/legalbenchrag_docname_legal_hyde",
        help="Base directory to save the output run folder."
    )
    parser.add_argument(
        "--cache-name", type=str, default="ai_cache_docname_legal_hyde",
