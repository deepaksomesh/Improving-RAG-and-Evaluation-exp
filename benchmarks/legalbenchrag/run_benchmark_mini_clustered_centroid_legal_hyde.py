...
async def run_strategy(
    ...
    # Limit concurrent API calls to avoid httpx socket timeouts when processing hundreds of queries
    ...
    async def run_query(qa_gt: QAGroundTruth) -> Tuple[QAGroundTruth, List[RetrievedSnippet]]:
        async with query_semaphore:
            # Find dataset tag (the existing script sets t.tags = [dataset_name])
            dataset_name = "unknown"
            valid_datasets = ["contractnli", "maud", "cuad", "privacy_qa"]
            ...
            # We MUST retain the original query because it contains the crucial entity names (e.g. "WECC", "Apple") 
            # needed to route to the correct document among the 72 corpus files!
...
def setup_and_load_data(max_tests: int, sort_by_doc: bool, seed: int) -> Tuple[List[Document], List[QAGroundTruth], List[float]]:
    ...
    for dataset_name, weight in benchmark_name_to_weight.items():  # TODO: benchmark_name_to_weight
        benchmark_file = f"./legalbenchragmini/benchmark/{dataset_name}.json"
    ...
    for doc_path in sorted(list(used_doc_paths)):
        full_path = f"./legalbenchragmini/corpus/{doc_path}"
...
class ClusteredCentroidBaselineRetrievalMethod(BaselineRetrievalMethod):
    ...
    async def sync_all_documents(self) -> None:
        ...
        chunking_params = {
            "strategy_name": "rcts", # FORCE standard chunking to remove SAC summaries
        ...
        for chunk in all_chunks:
            ...
            chunk.content = f"Document: {file_name}\n\n{chunk.content}"
            ...
            try:
                ...
                class CurrentCentroidMethod(ClusteredCentroidBaselineRetrievalMethod):
                    def __init__(self, retrieval_strategy, cache_dir=None):
                        super().__init__(retrieval_strategy, cache_dir, alpha=alpha)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results-dir", type=str, default="./results/legalbenchrag_clustered_centroid_legal_hyde",
