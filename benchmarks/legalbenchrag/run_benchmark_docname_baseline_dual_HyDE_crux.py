...
async def run_strategy(
    ...
    async def run_query(qa_gt: QAGroundTruth) -> Tuple[QAGroundTruth, List[RetrievedSnippet], Dict[int, Dict[str, float]]]:
        async with query_semaphore:
            ...
            try:
                # Specific Legal-HyDE Generation (Keyword Generation)
                hypothesis = await generate_specific_legal_hypothesis(qa_gt.query, dataset_name)
                
                # Search the vector DB using text concatenation (Keyword-stuffed query)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results-dir", type=str, default="./results/legalbenchrag_docname_baseline_dual_hyde_crux",
