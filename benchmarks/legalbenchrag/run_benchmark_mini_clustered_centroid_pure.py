...
def setup_and_load_data(max_tests: int, sort_by_doc: bool, seed: int) -> Tuple[List[Document], List[QAGroundTruth], List[float]]:
    ...
    for dataset_name, weight in benchmark_name_to_weight.items():  # TODO: benchmark_name_to_weight
        benchmark_file = f"./legalbenchragmini/benchmark/{dataset_name}.json" if use_mini else f"./data/benchmarks/{dataset_name}.json"
    ...
    for doc_path in sorted(list(used_doc_paths)):
        full_path = f"./legalbenchragmini/corpus/{doc_path}" if use_mini else f"./data/corpus/{doc_path}"
...
from sac_rag.data_models import QueryResponse

GLOBAL_RAW_EMBEDDINGS_CACHE = {}

class ClusteredCentroidBaselineRetrievalMethod(BaselineRetrievalMethod):
    def __init__(self, retrieval_strategy, cache_dir=None, alpha=1.0, db_name=None):
        super().__init__(retrieval_strategy, cache_dir, db_name=db_name)
        self.alpha = alpha

    async def sync_all_documents(self) -> None:
        stats_tracker.start_timer('chunking_and_summarization')
        print(f"ClusteredCentroidBaseline (alpha={self.alpha}): Calculating chunks using strategy '{self.retrieval_strategy.chunking_strategy.strategy_name}'...")

        chunking_params = {
            "strategy_name": "rcts", # FORCE standard chunking to remove SAC summaries completely
        ...
        print(f"ClusteredCentroidBaseline: Created {len(all_chunks)} chunks.")
        ...
        progress_bar = tqdm(total=len(all_chunks), desc="ClusteredCentroidBaseline: Processing Embeddings", ncols=100)
        ...
        print(f"ClusteredCentroidBaseline: Performing centroid anchoring with alpha={self.alpha}...")
        ...
        print(f"ClusteredCentroidBaseline: Start indexing anchored embeddings into SQLite...")
        ...
        print(f"ClusteredCentroidBaseline: Finished indexing {len(self.embedding_infos)} embeddings.")
...
async def main(args):
    ...
    print(f"Starting Legalbench-RAG Clustered Centroid benchmark run at: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for alpha in alpha_values:
        ...
        for i, config_path in enumerate(args.retrieval_configs):
            ...
            try:
                ...
                class CurrentCentroidMethod(ClusteredCentroidBaselineRetrievalMethod):
                    def __init__(self, retrieval_strategy, cache_dir=None, db_name=None):
                        super().__init__(retrieval_strategy, cache_dir, alpha=alpha, db_name=db_name or args.cache_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results-dir", type=str, default="./results/legalbenchrag_clustered_centroid_pure",
        help="Base directory to save the output run folder."
    )
    parser.add_argument(
        "--cache-name", type=str, default="ai_cache_clustered_centroid_pure",
        help="Custom name for the AI embedding cache and SQLite database to prevent overwriting between methods."
    )
    parser.add_argument(
        "--use-mini-dataset", action="store_true",
        help="Use the mini dataset instead of the full corpus."
    )
