...
async def run_strategy(
    ...
    # Limit concurrent API calls to avoid httpx socket timeouts
    ...
    async def run_query(qa_gt: QAGroundTruth) -> Tuple[QAGroundTruth, List[RetrievedSnippet]]:
        async with query_semaphore:
            ...
            # Combine original query (entity anchor) + boilerplate hypothesis
...
def create_summary_row(idx: int, config_path: str, strategy: Any, result: BenchmarkResult, top_k: int) -> Dict[str, Any]:
    ...
    elif type(strategy).__name__ == "TwoStageRetrievalStrategy":
        row.update({
            "method": "two_stage",
        })
    else:
...
async def main(args):
    ...
    # 3. Ablation Loop for summary_top_n
    for summary_top_n in [1, 3, 5]:
        n_results_dir = os.path.join(results_dir, f"summary_top_n_{summary_top_n}")
        os.makedirs(n_results_dir, exist_ok=True)
        print(f"\n=======================================================")
        print(f"Starting Sweep for Summary Top-N = {summary_top_n}")
        print(f"=======================================================")

        # 4. Loop through each provided configuration file
        for i, config_path in enumerate(args.retrieval_configs):
            ...
            try:
                ...
                # Override the summary_top_n from config for this ablation run
                if hasattr(strategy, 'summary_top_n'):
                    strategy.summary_top_n = summary_top_n
                else:
                    print(f"Strategy {type(strategy)} does not have summary_top_n. Skipping ablation override.")
                ...
                for k, result in results_by_k.items():
                    ...
                    result_filename = os.path.join(n_results_dir, f"{i}_{config_basename}_k{k}.json")
                    with open(result_filename, "w", encoding='utf-8') as f:
                        f.write(result.model_dump_json(indent=2))

                    # Prepare the DETAILED summary row
                    row = create_summary_row(i, config_path, strategy, result, k)
                    row["summary_top_n"] = summary_top_n # Add this so it's in the CSV
                ...
                traceback.print_exc()
                summary_rows.append(
                    {"config_file": config_path, "summary_top_n": summary_top_n, "recall": "ERROR", "precision": "ERROR", "f1_score": "ERROR"})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run LegalBench-RAG-mini Benchmark Evaluation")
    parser.add_argument(
    ...
    parser.add_argument(
        "--results-dir", type=str, default="./results/legalbenchrag_two_stage",
