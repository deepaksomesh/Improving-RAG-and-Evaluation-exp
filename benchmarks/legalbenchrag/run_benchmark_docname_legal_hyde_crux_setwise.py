...
from sac_rag.utils.ai import ai_embedding, AIEmbeddingType, set_ai_cache_name, get_ai_connection
import sqlite3
import sqlite_vec
...
async def setwise_compare(query: str, candidates: List[RetrievedSnippet]) -> int:
    ...
    for attempt in range(3):
        try:
            ...
            print(f"Setwise LLM Error: {e}")
            await asyncio.sleep(1)

    return 0

async def setwise_heapify_down(arr: List[RetrievedSnippet], n: int, i: int, query: str, c: int = 4):
    while True:
        candidates_indices = [i]
        for child_offset in range(1, c + 1):
            child_idx = c * i + child_offset
            if child_idx < n:
                candidates_indices.append(child_idx)
        
        if len(candidates_indices) == 1:
            break
            
        candidates = [arr[idx] for idx in candidates_indices]
        winner_local_idx = await setwise_compare(query, candidates)
        
        if winner_local_idx == 0:
            break
            
        winner_global_idx = candidates_indices[winner_local_idx]
        arr[i], arr[winner_global_idx] = arr[winner_global_idx], arr[i]
        i = winner_global_idx


async def setwise_heapsort(query: str, arr: List[RetrievedSnippet], k: int, c: int = 4) -> List[RetrievedSnippet]:
    """
    Sorts top-k documents from arr using a c-ary Setwise Heap Sort.
    """
    n = len(arr)
    if n == 0:
        return []
        
    # 1. Build heap (bottom up)
    last_non_leaf = (n - 2) // c
    for i in range(last_non_leaf, -1, -1):
        await setwise_heapify_down(arr, n, i, query, c)
        
    # 2. Extract max k times
    results = []
    extract_count = min(k, n)
    for i in range(extract_count):
        results.append(arr[0])
        arr[0], arr[n - 1 - i] = arr[n - 1 - i], arr[0]
        await setwise_heapify_down(arr, n - 1 - i, 0, query, c)
...
async def run_strategy(
    ...
    async def run_query(qa_gt: QAGroundTruth) -> Tuple[QAGroundTruth, List[RetrievedSnippet], Dict[int, Dict[str, float]]]:
        async with query_semaphore:
            ...
            try:
                # First Stage: DocName + Legal-HyDE
                ...
                first_stage_snippets = query_response.retrieved_snippets

                # Second Stage: Setwise Reranking
                max_k = max(final_k_values)
                retrieved_snippets = await setwise_heapsort(qa_gt.query, first_stage_snippets, max_k, c=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results-dir", type=str, default="./results/legalbenchrag_docname_legal_hyde_crux_setwise",
