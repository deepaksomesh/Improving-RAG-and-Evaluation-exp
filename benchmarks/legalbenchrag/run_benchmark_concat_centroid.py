...
class DocnameHydeRetrievalMethod(BaselineRetrievalMethod):
    async def sync_all_documents(self) -> None:
        ...
        # NO DOCNAME INJECTION - Pure Centroid Concatenation Approach
        ...
        print(f"ConcatCentroid: Performing centroid concatenation...")
        ...
        final_embeddings = np.concatenate((emb_np, centroids), axis=1).tolist()

        print(f"ConcatCentroid: Start indexing 3072-dimensional embeddings into SQLite...")
    ...
    async def query(self, query: str) -> 'sac_rag.data_models.QueryResponse':
        ...
        # Concatenate query with itself to match 3072 dimensions
        # We need to re-normalize the concatenated vector so cosine similarity works properly
        q_emb_concat = np.concatenate((q_emb, q_emb))
        q_emb_concat = q_emb_concat / np.linalg.norm(q_emb_concat)
        
        query_embedding_bytes = serialize_f32(q_emb_concat.tolist())
...
async def run_strategy(
    ...
    query_semaphore = asyncio.Semaphore(20)
    ...
    async def run_query(qa_gt: QAGroundTruth) -> Tuple[QAGroundTruth, List[RetrievedSnippet]]:
        async with query_semaphore:
            ...
            try:
                # Pure Centroid Concatenation Approach - NO HyDE
