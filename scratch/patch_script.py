...
with open('benchmarks/legalbenchrag/run_benchmark.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

new_lines = []
for line in lines:
    if 'from sac_rag.methods.baseline import BaselineRetrievalStrategy' in line:
        new_lines.append(line.replace('BaselineRetrievalStrategy', 'BaselineRetrievalStrategy, BaselineRetrievalMethod, serialize_f32, EmbeddingInfo'))
    elif line == '# --- Pydantic Models for this Benchmark\'s Evaluation Logic ---\n':
        # Inject our custom class here!
        docname_class = """
import sys
import pathlib
...
"""
        new_lines.append(docname_class)
    else:
        new_lines.append(line)

with open('benchmarks/legalbenchrag/run_benchmark_mini_docname.py', 'w', encoding='utf-8') as f:
