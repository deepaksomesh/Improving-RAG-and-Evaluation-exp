...
class AIModel(BaseModel):
    company: Literal["openai", "anthropic", "openrouter"]
    model: str
...
class AIEmbeddingModel(BaseModel):
    company: Literal["openai", "cohere", "voyageai", "huggingface", "google"]
    model: str
    ...
    def max_batch_tokens(self) -> int:
        if self.company in ["openai", "google"]:
            return 250000
        else:
...
class AIRerankModel(BaseModel):
    company: Literal["cohere", "voyageai", "huggingface", "openrouter"]
    model: str
...
os.makedirs(os.environ.get("SAC_CACHE_DIR", f"{Path.cwd()}/data/cache"), exist_ok=True)
cache = dc.Cache(
    os.environ.get("SAC_CACHE_DIR", f"{Path.cwd()}/data/cache") + "/ai_cache.db",
    size_limit=50 * (1024 ** 3)  # 50 GB size limit to prevent evicting massive corpus embeddings
)

def set_ai_cache_name(db_name: str):
    """Dynamically switch the diskcache database file name."""
    global cache
    cache = dc.Cache(
        os.environ.get("SAC_CACHE_DIR", f"{Path.cwd()}/data/cache") + f"/{db_name}.db",
...
class AIConnection:
    ...
    def __init__(self) -> None:
        self.shared_httpx_async_client_for_supported_libs = httpx.AsyncClient(
            timeout=httpx.Timeout(180.0, connect=10.0, read=160.0),
        ...
        if credentials.ai.openai_base_url:
            openai_kwargs["base_url"] = credentials.ai.openai_base_url
            
        self.openai_client = AsyncOpenAI(**openai_kwargs)
        
        self.openrouter_client = None
        if credentials.ai.openai_openrouter_api_key:
            openrouter_kwargs = {
                "api_key": credentials.ai.openai_openrouter_api_key.get_secret_value(),
                "http_client": self.shared_httpx_async_client_for_supported_libs
            }
            if credentials.ai.openrouter_base_url:
                openrouter_kwargs["base_url"] = credentials.ai.openrouter_base_url
            self.openrouter_client = AsyncOpenAI(**openrouter_kwargs)
    ...
    if isinstance(model, AIModel):
        if model.company == "anthropic":
            try:
                ...
                return int(len(s) / 4)
        elif model.company in ["openai", "openrouter"]:
            try:
                encoding = tiktoken.encoding_for_model(model.model)
                return len(encoding.encode(s))
            except Exception:
                logger.debug(f"Tiktoken model {model.model} not found, estimating tokens.")
                return int(len(s) / 4)
            ...
            for i in range(num_ratelimit_retries):
                try:
                    ...
                    def ai_message_to_openai_message_param(
                        ...
                        raise AIValueError(f"Unsupported message role for OpenAI/OpenRouter: {message.role}")

                    if i > 0:
                        logger.debug(f"{model.company}: Trying again after RateLimitError...")
                    
                    client = ai_conn.openrouter_client if model.company == "openrouter" else ai_conn.openai_client
                    if client is None:
                        raise ValueError(f"Client for {model.company} is not initialized. Please set API key in credentials.")

                    response = await client.chat.completions.create(
                        model=model.model,
                        messages=[ai_message_to_openai_message_param(m) for m in messages],
...
def get_embeddings_cache_key(
        model: AIEmbeddingModel, text: str, embedding_type: AIEmbeddingType, normalize: bool = True
) -> str:
    prefix = "_v4_norm" if normalize else "_v4_unnorm"
    key = f"{prefix}||||{model.company}||||{model.model}||||{embedding_type.name}||||{hashlib.md5(text.encode('utf-8', 'replace')).hexdigest()}"
    return key
...
async def ai_embedding(
    ...
    for i, text in enumerate(texts):
        if not text or not isinstance(text, str):
            continue
        cache_key = get_embeddings_cache_key(model, text, embedding_type, normalize)
    ...
    def _cache_processed_batch(proc_embeddings: List[List[float]], start_idx: int):
        for batch_i, new_embedding in enumerate(proc_embeddings):
            global_i = start_idx + batch_i
            original_idx = indices_to_fetch_map[global_i]
            text_embeddings_map[original_idx] = new_embedding

            text_for_cache = texts[original_idx]
            cache_key = get_embeddings_cache_key(model, text_for_cache, embedding_type, normalize)
    ...
    async def process_batch(batch_to_process: List[str]):
        ...
        for i_retry in range(num_ratelimit_retries):
            try:
                if model.company in ["openai", "google"]:
                    # Proactive rate limiting to prevent massive 429 backoff delays
                    async with ai_conn.openai_semaphore_instance:
                        tpm = model.ratelimit_tpm * RATE_LIMIT_RATIO
                        rpm = getattr(model, 'ratelimit_rpm', 3000) * RATE_LIMIT_RATIO
                        token_estimate = sum(len(t) // 3 for t in batch_to_process)
                        expected_wait_tpm = token_estimate / (tpm / 60) if tpm > 0 else 0
                        expected_wait_rpm = 1 / (rpm / 60) if rpm > 0 else 0
                        await asyncio.sleep(max(expected_wait_tpm, expected_wait_rpm, 1.0)) # Force at least 1s wait for OpenRouter
                        
                    client = ai_conn.openai_client
                    model_id = model.model
                    if model.company == "google" and getattr(ai_conn, "openrouter_client", None) is not None:
                        client = ai_conn.openrouter_client
                        # Openrouter uses "google/gemini-embedding-001" or just "gemini-embedding-001" depending on config,
                        # let's try with prefix if it doesn't already have one, or just let user configure it correctly.
                        # The user said they updated the credentials to configure gemini-embedding-001.
                        # OpenRouter expects "google/gemini-embedding-001" usually, but we'll use "google/" + model.model
                        model_id = f"google/{model.model}" if not model.model.startswith("google/") else model.model
                    
                    response = await client.embeddings.create(input=batch_to_process, model=model_id, encoding_format="float")
                    if not response.data:
                        raise ValueError(f"API returned an empty or invalid response. Expected data array. Response: {response.model_dump() if hasattr(response, 'model_dump') else response}")
                ...
                # Catch all exceptions (including openai.APIStatusError, VoyageAI errors, etc.) to ensure we retry on 429s
        ...
        if normalize:
            normalized_batch = []
            for emb in embeddings_for_batch:
                norm = sum(x * x for x in emb) ** 0.5
                if norm > 0:
                    normalized_batch.append([x / norm for x in emb])
                else:
                    normalized_batch.append(emb)
    ...
    for text_to_embed in required_texts_list:
        ...
        if (current_batch and len(current_batch) >= model.max_batch_len) or \
            ...
            start_idx = len(all_new_embeddings)
            processed_embeddings = await process_batch(current_batch)
            all_new_embeddings.extend(processed_embeddings)
            
            # Immediately write this batch to the cache so progress isn't lost on error
            _cache_processed_batch(processed_embeddings, start_idx)
            
            # Explicit visual progress update for the user
            print(f"  -> Generated embeddings for batch of {len(current_batch)} items... ({len(all_new_embeddings)} / {len(required_texts_list)} total)", flush=True)
    ...
    # Assemble final result combining cache hits and newly fetched embeddings
...
def get_rerank_cache_key(
    ...
    md5_hasher = hashlib.md5()
    md5_hasher.update(b"v3_cache_bust")
    md5_hasher.update(model.model_dump_json().encode())
...
async def ai_rerank(
    ...
    if cached_full_reranking is not None:
        ...
        elif model.company == "openrouter":
            import httpx
            from sac_rag.utils.credentials import credentials
            key = credentials.ai.openai_openrouter_api_key.get_secret_value() # Assuming OpenRouter key is placed here
            for i_retry in range(num_ratelimit_retries):
                try:
                    rpm = model.ratelimit_rpm * RATE_LIMIT_RATIO
                    async with ai_conn.cohere_semaphore_instance:
                        if rpm > 0: await asyncio.sleep(60.0 / rpm)
                        async with httpx.AsyncClient() as client:
                            response = await client.post(
                                "https://openrouter.ai/api/v1/rerank",
                                headers={"Authorization": f"Bearer {key}", "Content-Type": "application/json"},
                                json={"model": model.model, "query": query, "documents": processed_texts, "top_n": len(processed_texts)},
                                timeout=60.0
                            )
                            if response.status_code == 200:
                                full_reranked_indices = [r["index"] for r in response.json().get("results", [])]
                                break
                            else:
                                raise httpx.HTTPError(f"Status {response.status_code}: {response.text}")
                except Exception as e:
                    logger.warning(f"OpenRouter Rerank Error: {e}")
                    if i_retry == num_ratelimit_retries - 1:
                        logger.error("Max retries for OpenRouter. Fallback to original order.")
                        full_reranked_indices = list(range(len(processed_texts)))
                        break
                    await asyncio.sleep(backoff_algo(i_retry))
            if full_reranked_indices is None: full_reranked_indices = list(range(len(processed_texts)))
...
async def generate_document_summary(
    ...
    if summarization_model.company not in ["openai", "openrouter"]:
        logger.error(f"Summarization only supports OpenAI/OpenRouter. Requested: {summarization_model.company}. Fallback.")
    ...
    if use_cache and cached_summary is not None:
        logger.info(f"Cache hit for summary from diskcache: {document_file_path}")
        stats_tracker.increment('summaries_from_cache')
        return cast(str, cached_summary)
        
    # Attempt to load from file-based cache as a fallback if not in diskcache
    try:
        ...
        summary_filename_txt = os.path.join(summaries_output_dir_base, dataset_name, f"{sanitized_file_name}_summary.txt")
        
        if use_cache and os.path.exists(summary_filename_txt):
            with open(summary_filename_txt, 'r', encoding='utf-8') as f:
                file_summary = f.read().strip()
                if file_summary:
                    logger.info(f"Loaded summary from txt file cache: {summary_filename_txt}")
                    cache.set(cache_key, file_summary)
                    stats_tracker.increment('summaries_from_cache')
                    return file_summary
    except Exception as e:
        logger.warning(f"Failed to load summary from txt file cache: {e}")
