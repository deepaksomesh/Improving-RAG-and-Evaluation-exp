...
class OpenAIGenerator(APIGenerator):
    ...
    def __init__(self, model_name: str, api_key: str, base_url: str | None = None, **kwargs):
        ...
        self.client = AsyncOpenAI(api_key=api_key, base_url=base_url, http_client=self._shared_httpx_client_for_instance)
...
def create_generator(args) -> BaseGenerator:
    ...
    if "gpt-" in model_name_lower or "openai/" in model_name_lower:
        ...
        base_url = credentials.ai.openai_base_url
        if base_url:
            os.environ["OPENAI_BASE_URL"] = base_url
            
        return OpenAIGenerator(model_name=args.model_name, api_key=api_key, base_url=base_url, **common_kwargs)
